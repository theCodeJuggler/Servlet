...
public class AccountController {
	...
	public String addSavingsAccount(
			...
			@RequestParam("balance") String initBal,
			Model model) {
		
		Map<String, Object> map = new HashMap<>();
		
		map.put("accountHolderName",customerName);	map.put("contactNo", contactNumber);
		map.put("email", email);	map.put("dateOfBirth", LocalDate.parse(dateOfBirth, DateTimeFormatter.ofPattern("yyyy-MM-dd")));
		map.put("address", address);	map.put("gender", gender);
		map.put("balance", Double.parseDouble(initBal));	map.put("nationality", nationality);
		map.put("salaried", Boolean.parseBoolean(salaried));
		
		boolean isDob = doCheck((LocalDate)map.get("dateOfBirth"));
		boolean check = ((double)map.get("balance")>=1000 )?true:false;
